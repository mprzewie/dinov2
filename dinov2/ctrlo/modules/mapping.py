...
from dinov2.ctrlo.nns.convenience import build_transformer_encoder
...
class MLPMapping(nn.Module):
    ...
    def forward(self, x: typing.FeatureExtractorOutput) -> typing.FeatureExtractorOutput:
        x_copy = typing.FeatureExtractorOutput(
...
class IdentityMapping(nn.Module):
    def forward(self, x: typing.FeatureExtractorOutput) -> torch.Tensor:
        return x
...
class EncCondnMapping(nn.Module):
    ...
    def forward(
        self,
        x: typing.FeatureExtractorOutput,
        language: torch.Tensor,
        point: torch.Tensor,
        mask: torch.Tensor,
    ) -> typing.FeatureExtractorOutput:
        x_copy = typing.FeatureExtractorOutput(
            features=x.features.clone(), positions=x.positions.clone()
        )
